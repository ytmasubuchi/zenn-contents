...
CPU実験(2〜6章)は `run_all.sh` を叩けば手元で再現できます。GPU実験(7章)は別スクリプトの `run_gpu.sh` で、NVIDIA GPUと `nvidia-container-runtime` が使える環境が必要です。
...
原因はドライバ不整合ではありません。同じGPUを常駐のvLLMサーバが使っていて、`nvidia-smi` 実測で空きVRAMがたったの305MiB(全体24564MiB)しかなかったのです(`results/e8_environment_probe.json`。なお同ファイルの使用量23768MiBと合計24564MiBは単純には足し合いません。ドライバの予約領域など、nvidia-smiの集計上の丸めによるものです)。
...
これはcuBLAS公式ドキュメントの規定[^cublas]に基づく話で、この記事が実測で確かめたのはあくまで同一機種内の反復です。A100で固定した結果がH100でどうなるかは実測しておらず、公式ドキュメントの記述をそのまま引いています。
速度を犠牲にした上で、です。

### TF32という、桁の違う変動要因

ここまでの不一致を絶対差で見ると、Conv2dの`grad_input`やatomicAdd系は10⁻⁷〜10⁻³のオーダー、`cumsum`も最大2.44×10⁻⁴でした。ULP差や相対差だけが数百万・数十%と大きく見えた箇所は、直前で見たとおりゼロ近傍の桁落ちによる見かけ上の拡大です。

...
CPU実験の大半(2〜6章、E1〜E6)は `run_all.sh` を叩けば手元で再現できます(4・5章で引用する誤差の追試 = E7は別スクリプトです)。GPU実験(7章)も別スクリプトの `run_gpu.sh` で、NVIDIA GPUと `nvidia-container-runtime` が使える環境が必要です。
...
- `LogisticRegression(random_state=42)` の係数 `coef_`。予測クラスは一致したのに、係数の浮動小数点値だけがズレました。ズレの大きさを追試で実測すると(`results/e7_ulp_diff.json`)、20要素すべてが不一致で、最大絶対差は3.8×10⁻⁴。要素単位の相対差は最大で約8.6%にも達します(絶対値が最小の係数の要素で発生)。丸めの最終ビットが動いた、というレベルの話ではありません
...
一方LogisticRegressionは反復的な最適化(ソルバー)で解に近づいていくので、バージョン間の実装変更で収束の道筋が微妙に変わると、着地点そのものが動きます。
実際、追試でソルバー(lbfgs)の反復回数 `n_iter_` を見ると、旧環境が30回、新環境が25回と、そもそも止まった場所が違いました。差の水準も、ソルバーの収束許容誤差(既定 `tol=1e-4`)と同じオーダーで、丸め誤差の範疇ではありません。収束の停止点そのものが変わったことは、ここまでは実測で裏づけられています。
もっとも、前述のとおりこの実験では複数ライブラリ(Python・NumPy・scikit-learn)が同時に変わっているので、どのライブラリの変更が引き金かまでは、この実験単体では特定していません。
...
ここまでの「一致」「不一致」は、すべてハッシュ比較の結果です。
ハッシュは1ビットでも違えば「不一致」としか教えてくれません。
差が最終ビット1つ分なのか、目で見てわかるレベルなのかは、ハッシュだけではわからないのです。

そこで、不一致になった項目の生の値を保存し、差の大きさそのものを追試で実測しました(`results/e7_ulp_diff.json`)。
アーキテクチャ間(x86_64 vs ARM64)で不一致だった6項目の内訳です。

| 項目 | 不一致要素の割合 | 最大の差 |
|---|---|---|
| `np.exp`(10万要素) | 4.5% | 1 ULP(最終ビットのみ。最大絶対差 4.4×10⁻¹⁶) |
| 行列積 512×512 | 87.9% | 相対 5.6×10⁻¹⁰(絶対差は最大 1.0×10⁻¹³。ULPで見ると最大262万ULPだが、これは値がほぼ0の要素の桁落ちによる見かけ上の拡大) |
| RF `feature_importances_` | 15/20 | 5 ULP |
| LR `coef_` | 19/20 | 540 ULP(相対 1.0×10⁻¹³) |
| PyTorch MLP loss曲線(50点) | 21/50 | 2 ULP(float32換算) |
| PyTorch MLP 最終パラメータ(193個) | 23.3% | 6 ULP(float32) |

読み方はこうです。
アーキ間の差は、今回の実測の範囲ではすべて丸め誤差の伝播レベル(数ULP〜相対10⁻¹⁰)に収まっていて、目で見てわかるほどの差は1つもありませんでした。
predictが返す予測クラスが動じなかったのは、まさにこのスケールの小ささゆえです。

行列積だけULPの数値が突出して大きく見えますが、これは実害の大きさとは別の話です。
絶対差そのものは最大でも1.0×10⁻¹³にとどまっていて、262万ULPという数字は、値がほぼゼロの要素で桁落ちが起き、ULP換算の分母が小さくなったことによる見かけ上の拡大です。

ここで前章の数字と並べてみます。
前章のバージョン間(旧環境 vs 新環境)のLogisticRegression係数の差は、最大で約8.6%でした。
同じ「coef_の不一致」でも、環境をまたぐだけの差(丸めの揺れ、相対10⁻¹³のオーダー)と、実装そのものが変わる差(最適化の着地点が動く、相対8.6%のオーダー)とでは、スケールが桁違いです。
「一致しなかった」という同じ言葉の中に、無視してよさそうな揺れと、無視できない移動が両方含まれている、ということです。

ただし、これを「小さい差だから無視してよい」と読み替えるのは早計です。
学習は同じ計算を何度も繰り返すので、初期の微小差を増幅することがありますし、決定境界のすぐ近くにあるサンプルなら1 ULPの差でも予測クラスが反転しえます。
差の大きさを実測したからこそ「今回は小さかった」と言えるのであって、「小さいはずだから測らなくてよい」わけではありません。
...
1回の実行で終わらせず、各設定を毎回新しいコンテナで独立に5回ずつ、計10回実行して追試もしました(`results/e6_repeat_summary.json`)。

結果は、各設定内の5回すべて、そして設定間(1 vs 8)でも比較12項目全部が、**ビット単位で一致**でした。
...
先に検証の限界を断っておきます。5章のQEMU実験と同じ立て付けです。
今回のGPU実測は、すべてこのRTX 4090一枚・この1組のバージョンの中での反復です。異なるGPU機種(A100やH100など)や異なるCUDA/cuDNNバージョンをまたいだ場合にどうなるかは実測していません。この章の後半で「A100とH100では結果が変わりうる」といった書き方をする箇所は、実測ではなく、NVIDIA公式のcuBLASドキュメント[^cublas]など一次情報源の記述に基づく理論です。実測と理論の境目は、逐一断ります。

### 罠: GPUが「見える」ことと「使える」ことは別

実測に入る前に、1つ実務的な罠を挙げておきます。
最初の実験は、GPU上で1件の測定も成功せずに終わりました。
原因はドライバ不整合ではありません。同じGPUを常駐のvLLMサーバが使っていて、`nvidia-smi` 実測で空きVRAMがたったの305MiB(全体24564MiB)しかなかったのです(`results/e8_environment_probe.json`。なお同ファイルの使用量23768MiBと空き305MiBの和は合計24564MiBに一致しませんが、この差分約491MiBは丸めではなく、ドライバの予約領域など集計に含まれない領域によるものです)。
...
幸い後日vLLMが停止して空きVRAMが約22.9GiB(22903MiB、`nvidia-smi`実測)に戻ったため、同じスクリプトをそのまま再実行して、以下の実測データを得ました。
...
- **`cumsum`**: 100万要素中47.0%〜50.5%が不一致。最大絶対差は2.44×10⁻⁴(`results/e8_ulp_diff.json`)。最大相対差28.9%・最大ULP差約455万という数字だけ見ると衝撃的ですが、これは5章で行列積の262万ULPを「値がほぼゼロの要素での桁落ちによる見かけ上の拡大」と扱ったのと同じ構図です。実際、ULP差が最大だった要素(index 425175)は `-0x1.15…p-13` と `-0x1.8a…p-14` という、どちらも絶対値の小さい値同士の差でした

ここで、この実験(E8a)では `cudnn.benchmark` が既定の `False` のままだったことに注目してください。
つまり「どの畳み込み実装を使うか」は3コンテナとも同じはずなのに、`grad_input` だけがビット単位で揺れました。
forward出力とweight勾配は完全一致したのに、です。
これは前節の理屈で言う非決定性その1(アルゴリズム選択)ではなく、その2――選ばれた実装内部のatomicAddなど――が単独で効いている実例と考えられます(cuDNNの内部実装まで踏み込んで確認したわけではないので、原因をatomicAddと断定はしません)。**cudnn.benchmark=Falseはアルゴリズム選択の揺れは止めますが、選ばれたアルゴリズムそのものの非決定性までは止めません。**
...
**同じGPUアーキテクチャ・同じSM数(実務上はほぼ「同じ機種」)・同じCUDA/cuDNN/PyTorchバージョンの中だけ**です。速度を犠牲にした上で、です。
この範囲は、cuBLAS公式ドキュメントなど一次情報源の規定[^cublas]に基づく話であり、この記事が実測で確かめたのはあくまで同一機種内の反復です。A100で固定した結果がH100でどうなるかは実測しておらず、公式ドキュメントの記述をそのまま引いています。

### TF32という、桁の違う変動要因

ここまでの不一致を絶対差で見ると、Conv2dの`grad_input`やatomicAdd系は10⁻⁷〜10⁻³のオーダー、`cumsum`も最大2.44×10⁻⁴でした。ULP差や相対差だけが数百万・数十%と大きく見えた箇所は、先ほど見たとおりゼロ近傍の桁落ちによる見かけ上の拡大です。
TF32(TensorFloat-32、Ampere以降のGPUが行列積や畳み込みを高速化するために使う、仮数部を10bitに削った内部表現)は、それとは桁が違います。
...
この章の冒頭で触れたとおりTF32の既定値は経路によって違うので、意図せず片方だけ有効なまま計算していないか、確認する価値があります。
...
- **512×512行列積(同一入力)**: 262144要素中235051要素(89.7%)が不一致、最大相対差2.57%(最大絶対差は6.1×10⁻⁵)。CPUのBLASとGPUのcuBLASは加算順序も丸めの実装も別物なので、5章で見たBLASの非結合性の議論が、アーキ内の話に留まらずCPU⇔GPU間でもそのまま成立します
- **MLP学習(同一初期値、50ステップ)**: 最終パラメータ193要素中47要素(24.4%)が不一致。ただし最大ULP差はわずか**5**でした

3つ目が意外でした。
生の行列積は89.7%の要素が不一致で、相対差も最大2.57%に達していました(ただし絶対差そのものは最大6.1×10⁻⁵で、これも小さい値どうしの比較で相対差が誇張されている面があります)。それでも、そこから50ステップ学習した後のパラメータは、たった5ULPしか離れていません。
...
なお環境を何も変えない場合(表の上2行)の一致は、この3分類の手前にある「同じバイナリに同じ入力を与えれば同じ出力が出る」という自明な決定性のはずです。CPU実験ではその通りでしたが、GPUのデフォルト設定は必ずしもここに当てはまりません。数千のスレッドが並列に動くぶん、バイナリと入力が同じでも実行順序(≒スケジューリング)まで同じとは限らず、atomicAddを使う演算では「同一プロセス内の反復」ですら結果が揺れることを7章の実測(E8a)が示しています。3分類の出番は、通常は環境の何かが変わってからですが、GPUのデフォルト設定に限っては、環境を何も変えていなくてもこの手前の段階で結果が揺れうる、という注意が要ります。

| 変えるもの | 乱数列(PRNG出力) | 浮動小数点計算・学習結果 |
|---|---|---|
| 同一環境でプロセス再実行 | 一致(CPU実測。同一バイナリ+同一入力の決定性) | 一致(CPU実測。同左)。GPUのデフォルト設定はこの限りでない(下のGPU行を参照) |
| コンテナ再作成(同一イメージ) | 一致(CPU実測。同上) | 一致(CPU実測。同上)。GPUのデフォルト設定はこの限りでない(下のGPU行を参照) |
...
| GPU(同一機種内の反復、実測) | Philoxのカウンタ生成は3コンテナで反復一致。ただしPyTorchが明文で保証している値ではない | デフォルト設定は非保証で、実測はまだら――cuBLAS行列積・Conv2d forward/weight勾配・MLPは一致、`grad_input`・`index_add_`/`scatter_add_`・`cumsum`は不一致(atomicAdd等が原因)。決定的モード+環境変数なら実測で完全一致(ただし`cumsum`は決定的実装がなくエラーで停止)。TF32 on/offは絶対差0.03に達し、これらのULPノイズより桁違いに大きい |
| GPU機種・CUDAバージョンをまたぐ場合(未実測) | — | 非保証(cuBLAS公式ドキュメントが明言)。今回の実測はRTX 4090一枚・同一バージョンの中に限られ、機種やバージョンをまたぐ比較はしていない |

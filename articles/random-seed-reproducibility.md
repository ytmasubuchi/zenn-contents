...
[^cpython]: CPython `Modules/_randommodule.c` の `genrand_res53`。 https://github.com/python/cpython/blob/main/Modules/_randommodule.c
...
3回実行した結果、**15項目すべてが完全に一致**しました(`results/e1_compare_*.json`、不一致0件)。
...
白状すると、前章の「3回実行」も実は毎回新しいコンテナを起動して測っています。
プロセス再実行より厳しい条件で、最初から一致していたわけです。
...
[^pyguarantee]: "The generator's random() method will continue to produce the same sequence when the compatible seeder is given the same seed." https://docs.python.org/3/library/random.html
...
[^nep19]: NEP 19 — Random number generator policy. https://numpy.org/neps/nep-0019-rng-policy.html なお同文書には例外条項もあり、`random()`・`integers()`・`bytes()` の3メソッドのストリーム互換だけは保証対象です。
...
面白いのはGeneratorです。
一致した項目のうち、生の整数列と一様乱数はNEP 19の例外条項(`integers()` と `random()` は互換保証)の範囲内なので、これは約束どおりの一致です。
一方、正規乱数の一致には何の保証もありません。
...
レイヤー2の答えはシンプルです。
**一致するかどうかは「運」ではなく「各ライブラリの互換性ポリシー」で決まる。**
...
「超越関数はアーキ間で一致しない」と一括りにはできず、関数ごとに違う、つまり事前に予測できないというのが実測から言えることです。
...
これは実測値です。
`1.0 + 1e16` の時点で、1.0は1e16の丸め単位より小さいため吸収されて消えます。
...
BLASの実装は、行列のサイズやCPUによっては、スレッド数に関わらず同じ分割・同じ加算順序に落ちることがあります。
今回の規模ではそこに収まった、ということです。
...
同じ配列を前から足すか、後ろから足すか、素朴にループで足すかで、総和のビット表現は3通りに分かれました(`results/e6_omp1.json`)。

```
前から(np.sum):  -0x1.2f21bfc75b33cp+7
後ろから:         -0x1.2f21bfc75b33fp+7
素朴なループ:     -0x1.2f21bfc75b29fp+7
```

順序が結果を変えること自体は、疑いようがありません。
...
4. スレッド数を固定する(`OMP_NUM_THREADS=1` が最も硬い)
